...
class AnalyticsScreen extends StatelessWidget {
      ...
      body: SafeArea(
        child: SingleChildScrollView(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              _SummaryCard(label: 'GPS Uptime', value: '94.2%',
                  icon: Icons.gps_fixed,
                  color: NavExaTheme.accentTeal,
                  bg: NavExaTheme.accentTeal.withOpacity(0.1)),
              const SizedBox(width: 12),
              _SummaryCard(label: 'Loss Events', value: '3',
                  icon: Icons.gps_off,
                  color: NavExaTheme.accentTerra,
                  bg: NavExaTheme.accentTerra.withOpacity(0.1)),
            ]),
            const SizedBox(height: 12),
            Row(children: [
              _SummaryCard(label: 'AI Coverage', value: '100%',
                  icon: Icons.psychology,
                  color: NavExaTheme.accentOlive,
                  bg: NavExaTheme.accentOlive.withOpacity(0.1)),
              const SizedBox(width: 12),
              _SummaryCard(label: 'Avg Accuracy', value: '±3.2m',
                  icon: Icons.my_location,
                  color: NavExaTheme.accentGold,
                  bg: NavExaTheme.accentGold.withOpacity(0.1)),
            ]),
            const SizedBox(height: 24),

            const Text('GPS Accuracy Over Time',
                style: TextStyle(color: NavExaTheme.textDark,
  ...
  Widget _buildAccuracyChart() {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(
            color: NavExaTheme.brand.withOpacity(0.05),
      ...
      child: Column(children: [
        Row(
          ...
          children: [
            ...
            Column(
              ...
              children: ['10','5','0'].map((l) => Padding(
                padding: const EdgeInsets.only(right: 6),
                child: Text(l, style: const TextStyle(
                    color: NavExaTheme.textLight, fontSize: 9)),
              )).toList(),
            ),
            // Bars
            Expanded(
        ...
        Row(children: [
          const SizedBox(width: 26),
          _LegendDot(color: NavExaTheme.accentTeal, label: 'Normal'),
          const SizedBox(width: 12),
          _LegendDot(color: NavExaTheme.accentTerra, label: 'Signal Loss'),
        ]),
      ]),
    );
  }
  ...
  List<Widget> _buildEventList() {
    final events = [
      {'time': '14:23:05', 'duration': '47s',
          'type': 'Tunnel — AI DR Active', 'status': 'Recovered'},
      {'time': '11:08:41', 'duration': '22s',
          'type': 'Urban Canyon — Multipath', 'status': 'Recovered'},
      {'time': '09:15:12', 'duration': '63s',
          'type': 'Tunnel — AI DR Active', 'status': 'Recovered'},
    ];
    return events.map((e) => Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: NavExaTheme.accentTerra.withOpacity(0.2)),
        boxShadow: [BoxShadow(
            color: NavExaTheme.accentTerra.withOpacity(0.05),
      ...
      child: Row(children: [
        Container(
          ...
          decoration: BoxDecoration(
              color: NavExaTheme.accentTerra.withOpacity(0.1),
              borderRadius: BorderRadius.circular(10)),
          child: const Icon(Icons.warning_amber,
              color: NavExaTheme.accentTerra, size: 18),
        ),
        const SizedBox(width: 12),
        ...
        Container(
          ...
          child: Text(e['status']!, style: const TextStyle(
...
class _LegendDot extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(children: [
      ...
      Text(label, style: const TextStyle(
          color: NavExaTheme.textMid, fontSize: 10)),
    ]);
  }
}
